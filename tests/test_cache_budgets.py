...
def test_chunk_lru_default_is_256mb_per_handle(monkeypatch):
    from ngllib.simulator.em import EMTiles
...
def test_renderer_no_longer_pins_the_gl_budget():
    """SimulatorRenderer used to default mesh_budget_bytes to 2 GB and pass it
    down, which would have overridden the env knob before it reached
    MeshRenderer. None means 'let MeshRenderer resolve it'."""
    import inspect

    from ngllib.simulator import SimulatorRenderer

    p = inspect.signature(SimulatorRenderer.__init__).parameters["mesh_budget_bytes"]
